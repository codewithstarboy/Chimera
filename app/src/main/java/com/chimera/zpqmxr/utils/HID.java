...
public class HID {
    ...
    private String kbbyteCode(@NonNull String... args) {
        String tmp;
        if(args.length == 1) {
            tmp = "send_keys \"\\\\x00\\\\x00\\\\x"+args[0]+"\\\\x00\\\\x00\\\\x00\\\\x00\\\\x00\"\n";
        } else if(args.length == 2) {
            tmp = "send_keys \"\\\\x"+args[1]+"\\\\x00\\\\x"+args[0]+"\\\\x00\\\\x00\\\\x00\\\\x00\\\\x00\"\n";
        } else {
            tmp = "send_keys \"\\\\x00\\\\x00\\\\x00\\\\x00\\\\x00\\\\x00\\\\x00\\\\x00\"\n";
        }
        return tmp;
    }
    ...
    private ArrayList<String> mbyteCode(String[] mbytes, int count) {
        ...
        for (int i = 0; i < count; i++) {
            String tmp = "send_mouse \"\\\\x" + mbytes[0] + "\\\\x" + mbytes[1] + "\\\\x" + mbytes[2] + "\\\\x" + mbytes[3] + "\"\n";
            tmpStr.add(tmp);
        }
        return tmpStr;
    }
    ...
    public void parse(@NonNull String str) {
        ...
        for (int a = 0; a < lines.length; a++) {
            ...
            if (a == 0 && (lines[a].startsWith("DEFAULTDELAY") || lines[a].startsWith("DEFAULT_DELAY"))) {
                ...
                try {
                    defdelay = Float.parseFloat(con.trim().split(" ")[0]) / 1000;
                ...
                try {
                    float delay = Float.parseFloat(con.trim().split(" ")[0]) / 1000;
                ...
                try {
                    int x = Integer.parseInt(con.trim().split(" ")[0]);
                    for (int i = 0; i < x; i++) {
                        parse(lines[a - 1]);
                    }
                } catch (NumberFormatException e) {
